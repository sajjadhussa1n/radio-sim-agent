def check_self_intersection_batch(reflection_points, RX_points, building_edges, reflection_wall_edges, tolerance=1e-6):
        """Vectorized self-intersection check using batch processing"""
        # Create all rays at once
        rays = [LineString([rp, rx]) for rp, rx in zip(reflection_points, RX_points)]

        # Create STRtree of all non-reflection edges
        edges = [
            LineString([e1, e2])
            for e1, e2 in building_edges
            if not (np.allclose(e1, reflection_wall_edges[0], atol=tolerance) and
                    np.allclose(e2, reflection_wall_edges[1], atol=tolerance))
        ]
        tree = STRtree(edges)

        # Batch query
        #print("returning query_bulk now...")
        #return tree.query_bulk(rays, predicate='intersects').any(axis=1)
        results = []
        for ray in rays:
            results.append(tree.query(ray, predicate='intersects').any())

        return np.array(results)

    # Constants (ensure float64 precision for 3D calculations)
    ...
    def process_wall(wall, the_walls, buildings, R_grid, T_horiz, T, union_excluded_dict, polygons, valid_mask):
            ...
            # --- Occlusion Check ---
            # For each candidate receiver where valid_reflection is True, check if either the segment from TX to P
            # or from P to the receiver is blocked by any building (other than the reflecting one).
